...
SRC_DIR				=		src
TEST_DIR			=		tests
GUI_DIR				=		dynamicLibrary/Guillaume/
MAT_DIR				=		dynamicLibrary/Matteo/
TOM_DIR				=		dynamicLibrary/Thomas/

AR					=		ar rcs
CC					=		g++
RM					=		rm -rf

LDFLAGS				+=		-L $(GUI_DIR) -lgui
LDFLAGS				+=		-L $(MAT_DIR) -lmat
LDFLAGS				+=		-L $(TOM_DIR) -ltom
...
CFLAGS				+=		-I $(INCL_DIR) -I dynamicLibrary/
...
$(NAME):			$(MAIN_OBJ) $(PROJ_OBJ)
					...
					cd $(MAT_DIR) && $(MAKE) lib
					cd $(TOM_DIR) && $(MAKE) lib
					$(CC) $(MAIN_OBJ) $(PROJ_OBJ) -o $(NAME) $(LDFLAGS) $(LDLIBS) -ldl \
...
lib_fclean:			lib_clean
					$(RM) $(LIB_NAME)
					cd $(GUI_DIR) && $(MAKE) lib_fclean
					cd $(MAT_DIR) && $(MAKE) lib_fclean
					cd $(TOM_DIR) && $(MAKE) lib_fclean
