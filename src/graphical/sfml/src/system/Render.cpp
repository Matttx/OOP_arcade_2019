...
sfml::system::Render::Render(engine::ecs::World& world, sf::RenderWindow& window) : ARender(world), _window(window)
{
}
    ...
    for (const auto& entity : entitiesSize) {
        if (entity.get().hasComponents<engine::component::AText>()) {
            ...
            sfmlText.text.setScale(
                size.width / sfmlText.text.getLocalBounds().width, size.height / sfmlText.text.getLocalBounds().height);
        }
        if (entity.get().hasComponents<engine::component::ARender>()) {
            auto& size = entity.get().getComponent<engine::component::Size>();
            auto& component = entity.get().getComponent<engine::component::ARender>();
            ...
            sfmlRender.sprite.setScale(size.width / sfmlRender.sprite.getLocalBounds().width,
