...
void sfml::system::Animations::update()
{
    ...
    for (auto& entity : entities) {
        ...
        if (currentTimeMs > animations.lastTimeMs + (unsigned int)(currentAnimation.speed)) {
            sfmlRender.srcRect.width = sfmlRender.texture.getSize().x / currentAnimation.frames;
            sfmlRender.srcRect.height = sfmlRender.texture.getSize().y / (int)(animations.list.size());
            sfmlRender.srcRect.left = animations.currentFrame * sfmlRender.srcRect.width;
            sfmlRender.srcRect.top = currentAnimation.row * sfmlRender.srcRect.height;
