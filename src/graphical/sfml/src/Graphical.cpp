...
sfml::Graphical::Graphical(engine::eventbus::EventBus &eventBus) : _eventBus(eventBus){
}
...
engine::component::AAudio sfml::Graphical::createAudio(ecs::Entity& entity, const std::vector<std::string>& paths) {
    _audios.emplace_back(entity, paths);
    return _audios.back();
}

engine::component::ARender sfml::Graphical::createRender(ecs::Entity& entity, const std::vector<std::string>& paths) {
